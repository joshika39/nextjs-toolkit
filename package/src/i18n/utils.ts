 ...
 * @param values
 */
function _formatTranslation(translation: string, values: Record<string, any> = {}) {
  for (const key in values) {
    const matches = translation.match(/{{(.*?);(.*?)}}/);
    if (matches) {
      const [fullMatch, pluralityKey, contents] = matches || [];
      const [_key, singular, plural] = contents.split(";");
      if(isNumber(values[key] && values[key] > 1)) {
        translation = translation.replace(`||${pluralityKey}||`, plural);
      } else {
        translation = translation.replace(`||${pluralityKey}||`, singular);
      }
      translation = translation.replace(fullMatch, `${values[key]}`);
    } else {
 ...
 */
export function formatTranslation(translation: string, values: Record<string, any> = {}): React.ReactNode {
 ...
 */
export function formatStringTranslation(translation: string, values: Record<string, any> = {}): string {
